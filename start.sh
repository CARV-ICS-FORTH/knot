...
if [[ -n $KARVDASH_OPENBIO_URL && -n $KARVDASH_OPENBIO_NAMESPACE ]]; then
    python manage.py createoauthapplication --name openbio --redirect-uri $KARVDASH_OPENBIO_URL/login/generic_oauth --secret-name karvdash-oauth-openbio --secret-namespace $KARVDASH_OPENBIO_NAMESPACE
fi
